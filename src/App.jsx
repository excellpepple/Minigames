...
export default function App() {
  // Initialize virtual cursor globally
  const { videoRef, cameraError, isLoaded, boot } = useVirtualCursor();
  
  // State for virtual cursor mode toggle (default to enabled)
  ...
  // Enable dwell-to-click functionality only when virtual cursor is enabled
  useDwellToClick(virtualCursorEnabled && isLoaded && boot === "ready");
  
  // Update cursor visibility based on toggle
  useEffect(() => {
  ...
  return (
    <ThemeProvider>
      ...
      {/* Global virtual cursor video element - can be styled by pages */}
      <video
        ref={videoRef}
        id="video"
        autoPlay
      ...
      {/* Global virtual cursor element - visibility controlled by toggle */}
      <div
        id="cursor"
        ...
        style={{
          ...
          zIndex: 60,
          opacity: virtualCursorEnabled && isLoaded && boot === "ready" ? 1 : 0,
      ...
      {/* Virtual Cursor Toggle Button - Fixed at bottom */}
      <button
        ...
        aria-label={virtualCursorEnabled ? "Switch to mouse/keyboard mode" : "Switch to virtual cursor mode"}
        title={virtualCursorEnabled ? "Switch to mouse/keyboard mode" : "Switch to virtual cursor mode"}
      >
        {virtualCursorEnabled ? (
          <>
            <svg
